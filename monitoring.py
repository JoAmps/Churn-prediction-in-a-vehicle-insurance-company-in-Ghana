...
def monitor_model_and_data():
    ...
    if latest_data == current_data:
        ...
        logging.info('New data available, therefore running training script on new data')    
        ...
        if current_recall_score<np.max(previous_recall_score)!=True:
            logging.info('Current recall score not less than previous score, model_drift not occurred') 
            logging.info('end process')   
            exit
        else:
            logging.info('Current recall score less than previous score, model_drift may have occurred') 
